...
Feature: Non repeated subAF feature
  ...
  @javascript
  Scenario: Creation of a non repeated subAF
  # Accès au datagrid
  ...
  # Sous-formulaires non répétés ordonnés suivant l'ordre alphabétique des identifiants
    And the row 1 of the "subAfNotRepeatedDatagrid" datagrid should contain:
  ...
  @javascript
  Scenario: Edition of a non repeated subAF
    Given I am on "af/edit/menu/id/4"
    And I wait for the page to finish loading
    And I open tab "Composants"
    And I open collapse "Sous-formulaires non répétés"
