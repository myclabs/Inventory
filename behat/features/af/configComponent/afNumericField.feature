...
Feature: AF numeric field feature
  ...
  # Ajout, saisie correcte
    ...
    And I select "Masqué" in radio "Visibilité"
    And I select "Désactivé" in radio "Activation"
    And I select "Obligatoire" in radio "Obligatoire ?"
    And I fill in "numericFieldDatagrid_unit_addForm" with "kg_co2e.m3^-1"
    And I select "Non modifiable" in radio "Unité modifiable ?"
    And I select "Masquée" in radio "Aff. incertitude"
    ...
    And I select "Affiché" in radio "Rappel val. défaut"
    And I click "Valider"
    Then the following message is shown and closed: "Ajout effectué."
  # Vérification ligne bien ajoutée
    And the "numericFieldDatagrid" datagrid should contain a row:
