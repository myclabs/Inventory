...
class Classification_Datagrid_ConsistencyController extends UI_Controller_Datagrid
{
    ...
    public function getelementsAction()
    {
        ...
        foreach (Axis::loadList() as $axis) {
            if (!$axis->hasMembers()) {
                $listAxisWithoutMember[] = $this->translationHelper->toString($axis->getLabel());
            } else {
                ...
                foreach ($axis->getMembers() as $member) {
                    if ($narrowerAxis !== null) {
                        ...
                        if (count($intersectMemberNarrowerMembers) < 1) {
                            ...
                            $listAxisWithMemberNotLinkedToNarrower[$axis->getId()][$narrowerAxis->getId()][] = $this->translationHelper->toString($member->getLabel());
                        }
                    }
                    foreach ($broaderAxes as $broaderAxis) {
                        ...
                        if (count($intersectMemberBroaderMembers) !== 1) {
                            ...
                            $listAxisWithMemberNotLinkedToBroader[$axis->getId()][$broaderAxis->getId()][] = $this->translationHelper->toString($member->getLabel());
                        }
                    }
                }
            }
            ...
            foreach ($contextIndicatorAxes as $contextIndicatorAxis) {
                foreach ($contextIndicatorAxes as $contextIndicatorAxisVerif) {
                    if (($contextIndicatorAxis !== $contextIndicatorAxisVerif)
                        ...
                        $contextIndicatorErrors[] = '(' . $this->translationHelper->toString($contextIndicatorAxis->getLabel()) . ' - ' . $this->translationHelper->toString($contextIndicatorAxisVerif->getLabel()) . ')';
                    }
                }
            }
        ...
        foreach ($listAxisWithMemberNotLinkedToNarrower as $axisId => $members) {
            ...
            $data['fail'] .= $this->translationHelper->toString($axis->getLabel()) . ' : { ';
            foreach ($members as $narrowerAxisId => $refMember) {
                $narrowerAxis = Axis::load($narrowerAxisId);
                $data['fail'] .= $this->translationHelper->toString($narrowerAxis->getLabel()) . ' : [' . implode(', ', $refMember) . '], ';
            }
        ...
        foreach ($listAxisWithMemberNotLinkedToBroader as $axisId => $members) {
            ...
            foreach ($members as $broaderAxisId => $refMember) {
                ...
                $data['fail'] .= $this->translationHelper->toString($broaderAxis->getLabel()) . ' : [' . implode(', ', $refMember) . '], ';
            }
            $data['fail'] = substr($data['fail'], 0, -2);
            $data['fail'] .= ' }, ';
        }
        ...
        foreach ($listContextIndicatorsWithLinkedAxes as $contextIndicatorArray) {
            $data['fail'] .= $this->translationHelper->toString($contextIndicatorArray['contextIndicator']->getContext()->getLabel()) . ' - ' .
