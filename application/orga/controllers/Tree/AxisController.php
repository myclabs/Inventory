...
class Orga_Tree_AxisController extends UI_Controller_Tree
{
    ...
    public function getnodesAction()
    {
        ...
        foreach ($axes as $axis) {
            $this->addNode(
                $axis->getRef(),
               '<b>'.$this->translationHelper->toString($axis->getLabel()).'</b> <i>('.$axis->getRef().')</i>',
    ...
    public function addnodeAction()
    {
        ...
        if (empty($this->_formErrorMessages)) {
            ...
            $this->translationHelper->set($axis->getLabel(), $this->getAddElementValue('addAxis_label'));
            ...
            if ($axis->getDirectNarrower() === null) {
                ...
                $this->message = __('UI', 'message', 'added', [
                    'AXIS' => $this->translationHelper->toString($axis->getLabel()),
                    'PARENT' => $this->translationHelper->toString($axis->getDirectNarrower()->getLabel()),
                ]);
            }
        }
    ...
    public function getlistparentsAction()
    {
        ...
        foreach ($organization->getFirstOrderedAxes() as $axis) {
            $this->addElementList($axis->getRef(), ' '.$this->translationHelper->toString($axis->getLabel()));
        }
    ...
    public function getlistsiblingsAction()
    {
        ...
        foreach ($axes as $siblingAxis) {
            if ($siblingAxis !== $axis) {
                $this->addElementList(
                    $siblingAxis->getRef(),
                    $this->translationHelper->toString($siblingAxis->getLabel()).' ('.$siblingAxis->getRef().')'
                );
            }
        }
...
//        }
        ...
        if (empty($this->_formErrorMessages)) {
            ...
            if ($this->translationHelper->toString($axis->getLabel()) !== $newLabel) {
